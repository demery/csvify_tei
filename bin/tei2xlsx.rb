...
headers = [ "Repository","Title","Call No.","Language(s)","Origin","Date","Place","Subject","Genre","Front Cover","OPenn Navigation","OPenn Data","Ebook","Page Turning","Record","Video" ]
...
# Add a worksheet
wkbk.add_worksheet name: 'mss' do |wksh|

  # add the header row
  wksh.add_row headers
  ...
  # Make all the columns 30 points(?) wide; otherwise, we get columns
  # that are too wide.
  # Create an array of 30s; one for each header [ 30, 30, ... ]
  widths = headers.map { |h| 30 }
  # * splat operator converts the array to a series of arguments
